...
function App() {
  ...
  const animations = [
    'scale-110 rotate-3 duration-100',
    'scale-110 -rotate-3 duration-100',
    'scale-105 rotate-1 duration-100',
    'scale-105 -rotate-1 duration-100'
  ];
  ...
  const spin = () => {
    ...
    const spinInterval = setInterval(() => {
      ...
      counter++;
      if (counter >= 8) { // Reduced from 12 to 8 iterations
        clearInterval(spinInterval);
        setIsSpinning(false);
        const newResult = { 
          tier: finalTier, 
          animation: 'scale-100 duration-300', // Reduced from 500 to 300ms
          timestamp: Date.now()
        };
        setResult(newResult);
